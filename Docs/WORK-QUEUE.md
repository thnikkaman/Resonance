...
- Baseline: the old idle-preset proof of concept rendered black and reported `GL_INVALID_FRAMEBUFFER_OPERATION`; the replacement now renders to the active GLKView framebuffer through projectM’s FBO API. The renderer uses the actual drawable size and a 24×24 projectM mesh while the framebuffer path is stabilized. Presets and texture dependencies are namespaced under the app bundle’s `ProjectMD/` directory.
