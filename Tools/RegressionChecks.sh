...
assert 'await library.scanSharedMusicFolder(forceMetadataRefresh: false)' not in now_playing
...
assert 'var directories = [folderURL]' in lyrics_service
assert 'contentsOfDirectory(' in lyrics_service
...
assert 'record.fileName.flatMap { remoteOriginalFileName(fromPath: $0) }' in remote
