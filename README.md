...
- Preserve build 63 (`0.3.7.4`, source commit `c3c1ebc`) as the current stability and rollback reference while testing build 66. Any new playback or Streaming work must retain its responsive tab switching, Library/Streaming/Settings navigation, Now Playing behavior, audio playback, and confirmed 5.1 gapless behavior.
- Launch build 66 on SaiyanDenwa with **Experimental streaming gapless** enabled. Use remote MP3 and FLAC tracks with a following queue item and confirm the transition has no audible silence, the next track starts at its beginning, and the visible title/progress changes at the boundary. Repeat with the setting disabled to confirm stable single-item playback remains available.
- On build 66, test pause/resume, in-track seek, Next, Previous, repeat-one, repeat-all, play-next, add-to-queue, queue reordering, and toggle-off recovery around a remote boundary. Confirm tab switching and Streaming scrolling remain responsive while audio is playing.
- Retrieve a fresh `Documents/Resonance-Diagnostics.log` after the build-66 runtime test and correlate `remote.player.queueConfigured`, `remote.player.finished`, `remote.player.boundary.end`, `remote.player.endFallback`, and any new errors with the recording. Do not launch Resonance automatically or commit the copied diagnostics file.
- Complete the build-65 remote seek-display and accent-text acceptance on build 66 as well: elapsed and remaining labels must not exceed the active duration after fast-forwarding near or beyond the endpoint, ordinary seeks must remain accurate, and the text-color preference must not restore broad tab-tree redraws.
