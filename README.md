...
- Repair remote seek presentation by clamping elapsed time to the current track duration after AVPlayer updates and queue boundaries, then verify fast-forwarding never makes the labels exceed the duration while preserving accurate in-track seeking.
- Restore accent-text application without reintroducing root-wide settings observation: the **Apply theme color to text** preference must style intended ordinary text while tab switching, Streaming, Settings, and playback remain responsive.
