...
- Perform physical-device performance acceptance for build 62 while audio is stopped and playing: switch repeatedly among Library, Streaming, Settings, and Playing; confirm cached Library content stays visible during startup scanning; open local album detail and Now Playing; scroll a large Streaming catalog; expand/collapse Settings categories; edit Settings and hex fields; verify the opaque tab bar stays fixed without ghosting or covering content; verify the keyboard can be dismissed; and retrieve the now-clean diagnostics/resource logs after any slowdown or lockup. Inspect `library.scan.*`, `remote.playback.preparationDiscarded`, `artwork.local.thumbnail`, and `preparationMs` events if the delay remains.
