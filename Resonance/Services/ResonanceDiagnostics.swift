...
final class ResonanceDiagnostics: @unchecked Sendable {
  ...
  func recordDeferred(_ event: String, details: [String: String] = [:]) {
    ...
    recordDeferredAlways(event, details: details)
  }

  /// Records a low-volume performance boundary without blocking its caller.
  /// Use this for renderer events that must remain available even when the
  /// optional verbose diagnostics mode is disabled.
  func recordDeferredAlways(_ event: String, details: [String: String] = [:]) {
    let line = makeLine(event, details: details)
...
func resonance_diagnostics_record(
  ...
  // ProjectM invokes this callback from its display/preset/texture paths.
  // Opening and writing the diagnostics file synchronously here blocks the
  // same main/render transaction that presents frames and handles controls.
  ResonanceDiagnostics.shared.recordDeferredAlways(eventString, details: details)
}
