...
enum LyricsService {
  ...
  static func localDocument(
    ...
    return await Task.detached(priority: .utility) {
      ...
      let candidates: [(kind: String, load: () -> Data?)] = [
        ...
        ("directSibling", { track.fileURL.flatMap { coordinatedSiblingLRCData(beside: $0) } }),
  ...
  private static func coordinatedSiblingLRCData(beside audioURL: URL) -> Data? {
  ...
  private static func selectedFolderSiblingLRCData(for track: Track, selectedRoot: URL?) -> Data? {
    ...
    return coordinatedSiblingLRCData(beside: selectedAudioURL)
  }

  /// Last-resort lookup for a stale cached audio URL. The selected library
  /// folder is authoritative, so locate the exact same-basename LRC anywhere
  /// beneath it without requiring the cached audio path to share its root.
  private static func selectedFolderMatchLRCData(for track: Track, selectedRoot: URL?) -> Data? {
    ...
    return try? ExternalFileCoordinator.read(at: selectedRoot) { coordinatedRoot in
      ...
      while let candidate = enumerator?.nextObject() as? URL {
        guard candidate.pathExtension.caseInsensitiveCompare("lrc") == .orderedSame,
