...
final class PlayerController: NSObject, ObservableObject {
  ...
  /// Monotonic playback-start identity used by Now Playing to re-read local
  /// lyric files after a resume or backend restart.
  @Published private(set) var playbackRestartID = 0
  ...
  private func markPlaybackRestart() {
    playbackRestartID &+= 1
  }
  ...
  private func startPreparedRemoteSampleContiguousPlayback(
    ...
    do {
      ...
      isPlaying = autoPlay
      if autoPlay { markPlaybackRestart() }
  ...
  private func updateElapsedFromClock() {
    ...
    case .gapless:
      ...
      let effectiveRate = isCurrentAudiobook ? playbackRate : 1
      elapsed = min(
        duration,
        playbackAnchorElapsed
          + Date().timeIntervalSince(playbackAnchorDate) * effectiveRate
      )
    case .legacy:
