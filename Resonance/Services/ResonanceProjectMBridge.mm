...
namespace {
constexpr size_t kPCMCapacityFrames = 16384;
constexpr size_t kPCMMaximumChannels = 2;
constexpr size_t kPCMScratchScalars = kPCMCapacityFrames * kPCMMaximumChannels;
// ProjectMD's working renderer submits a 480-frame visualization window per
// display tick. Keep the queue bounded for recent-audio staging, but never
// hand the entire pending backlog to projectM: doing so makes one main-thread
// render call grow with audio starvation and competes with SwiftUI controls.
constexpr size_t kPCMVisualizationFrames = 480;
constexpr size_t kPCMVisualizationScalars = kPCMVisualizationFrames * kPCMMaximumChannels;

struct ResonancePCMStagingRing {
    std::array<float, kPCMScratchScalars> samples{};
    std::atomic<size_t> writeIndex{0};
    std::atomic<size_t> readIndex{0};
    std::atomic<size_t> latestChannels{1};
    ...
    void Reset() {
        ...
        latestChannels.store(1, std::memory_order_relaxed);
        submittedFrames.store(0, std::memory_order_relaxed);
        consumedFrames.store(0, std::memory_order_relaxed);
        droppedFrames.store(0, std::memory_order_relaxed);
    }

    void Submit(const float *samples, size_t frameCount, size_t channels) {
        const size_t safeChannels = std::clamp(channels, size_t{1}, kPCMMaximumChannels);
        submittedFrames.fetch_add(frameCount, std::memory_order_relaxed);

        size_t write = writeIndex.load(std::memory_order_relaxed);
        const size_t read = readIndex.load(std::memory_order_acquire);
        const size_t pendingScalars = std::min(write - read, kPCMScratchScalars);
        const size_t requestedScalars = frameCount * safeChannels;
        const size_t availableScalars = kPCMScratchScalars - pendingScalars;
        size_t acceptedScalars = std::min(requestedScalars, availableScalars);
        acceptedScalars -= acceptedScalars % safeChannels;
        if (acceptedScalars < requestedScalars) {
            droppedFrames.fetch_add((requestedScalars - acceptedScalars) / safeChannels,
                                    std::memory_order_relaxed);
        }

        const size_t firstCopy = std::min(acceptedScalars, kPCMScratchScalars - (write % kPCMScratchScalars));
        std::copy_n(samples, firstCopy, this->samples.begin() + (write % kPCMScratchScalars));
        std::copy_n(samples + firstCopy, acceptedScalars - firstCopy, this->samples.begin());
        write += acceptedScalars;
        latestChannels.store(safeChannels, std::memory_order_relaxed);
        writeIndex.store(write, std::memory_order_release);
    }

    size_t Drain(float *destination, size_t scalarCapacity, size_t& channels) {
        size_t read = readIndex.load(std::memory_order_relaxed);
        const size_t write = writeIndex.load(std::memory_order_acquire);
        const size_t pendingScalars = std::min(write - read, kPCMScratchScalars);
        channels = latestChannels.load(std::memory_order_relaxed);
        const size_t windowCapacity = std::min(scalarCapacity, kPCMVisualizationScalars);
        // Visualization audio is a recent window, not a lossless playback
        // queue. If rendering fell behind, discard stale samples before the
        // copy so projectm_pcm_add_float remains bounded at every frame.
        if (pendingScalars > windowCapacity) {
            read = write - windowCapacity;
        }
        size_t scalarCount = std::min(pendingScalars, windowCapacity);
        scalarCount -= scalarCount % channels;
        const size_t firstCopy = std::min(scalarCount, kPCMScratchScalars - (read % kPCMScratchScalars));
        std::copy_n(samples.begin() + (read % kPCMScratchScalars), firstCopy, destination);
        std::copy_n(samples.begin(), scalarCount - firstCopy, destination + firstCopy);
        read += scalarCount;
        readIndex.store(read, std::memory_order_release);
        const size_t frameCount = scalarCount / channels;
        consumedFrames.fetch_add(frameCount, std::memory_order_relaxed);
        return frameCount;
    }
};

std::atomic<bool> gNativeVisualizerAudioEnabled{false};
std::atomic<uint32_t> gPCMSubmitters{0};
ResonancePCMStagingRing gPCMStaging;

void DisablePCMStaging() {
    gNativeVisualizerAudioEnabled.store(false, std::memory_order_release);
    while (gPCMSubmitters.load(std::memory_order_acquire) != 0) {
        std::this_thread::yield();
    }
}

void RecordPCMStagingSummary() {
    char value[160];
    snprintf(value, sizeof(value),
             "submitted_frames=%llu consumed_frames=%llu dropped_frames=%llu",
             static_cast<unsigned long long>(gPCMStaging.submittedFrames.load(std::memory_order_relaxed)),
             static_cast<unsigned long long>(gPCMStaging.consumedFrames.load(std::memory_order_relaxed)),
             static_cast<unsigned long long>(gPCMStaging.droppedFrames.load(std::memory_order_relaxed)));
    resonance_diagnostics_record("projectm.pcm.staging_summary", "result", value);
}
}
...
@interface ResonanceProjectMBridge () {
    ...
    std::array<float, kPCMVisualizationScalars> _pcmScratch;
}
...
struct ResonanceNativeProjectM {
    ...
    std::array<float, kPCMVisualizationScalars> pcmScratch{};
};
...
extern "C" void resonance_native_projectm_render(ResonanceNativeProjectM *renderer) {
    if (!resonance_native_projectm_is_ready(renderer)) return;
    size_t channels = 1;
    const size_t frameCount = gPCMStaging.Drain(renderer->pcmScratch.data(),
                                                renderer->pcmScratch.size(),
                                                channels);
    if (frameCount > 0) {
        projectm_pcm_add_float(renderer->handle,
                               renderer->pcmScratch.data(),
...
std::vector<unsigned char> RasterizeLyricText(const char *utf8Text, int& width, int& height) {
    // Keep the rasterized glyph scale stable while giving long lyric lines
    // 50% more horizontal layout room than the original 1024px canvas.
    width = 1536;
    height = 256;
...
+ (void)submitPCM:(const float *)samples count:(NSUInteger)count channels:(NSUInteger)channels {
    ...
    gPCMSubmitters.fetch_add(1, std::memory_order_acq_rel);
    if (gNativeVisualizerAudioEnabled.load(std::memory_order_acquire)) {
        gPCMStaging.Submit(samples, count, channels);
    }
    gPCMSubmitters.fetch_sub(1, std::memory_order_release);
}
...
- (void)drawFrame {
    ...
    size_t channels = 1;
    const size_t frameCount = gPCMStaging.Drain(_pcmScratch.data(), _pcmScratch.size(), channels);
    if (frameCount > 0) {
        projectm_pcm_add_float(_instance, _pcmScratch.data(), static_cast<unsigned int>(frameCount),
                               channels == 1 ? PROJECTM_MONO : PROJECTM_STEREO);
    }
