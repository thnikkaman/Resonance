...
enum ResonanceAppInformation {
    static let displayName = "MeiKyo"
    static let displayNameWithKanji = "MeiKyo 鳴響"
    static let privacyPolicyURL = URL(string: "https://resonance-privacy.saiyansource-3119.chatgpt.site/")!
    static let projectMURL = URL(string: "https://github.com/projectM-visualizer/projectm")!
    static let projectMPresetsURL = URL(string: "https://github.com/projectM-visualizer/presets-cream-of-the-crop")!
}
...
final class AppSettings: ObservableObject {
    ...
    @AppStorage("settingsMusicLibraryExpanded") var settingsMusicLibraryExpanded = true
    @AppStorage("settingsVisualizerExpanded") var settingsVisualizerExpanded = true
    ...
    @AppStorage("settingsAboutExpanded") var settingsAboutExpanded = true
    @AppStorage("resonance.privacyPolicyAcknowledged") var privacyPolicyAcknowledged = false
    @AppStorage("showArtworkWarning") var showArtworkWarning = true
    @AppStorage("musicBrainzArtworkNoticeAcknowledged") var musicBrainzArtworkNoticeAcknowledged = false
    @AppStorage("flacAlert") var flacAlert = false
    @AppStorage("showFrameDiagnostics") var showFrameDiagnostics = false
    @AppStorage("showVisualizerFPSCounter") var showVisualizerFPSCounter = false
    @AppStorage(VisualizerTelemetryConfiguration.enabledKey) var visualizerTelemetryEnabled = false
    ...
    @AppStorage("resonance.projectmd.fullscreenLyricsEnabled") var projectMFullscreenLyricsEnabled = true
    @AppStorage("resonance.projectmd.shufflePresets") var projectMShufflePresets = true
    @AppStorage("resonance.projectmd.autoCyclePresets") var projectMAutoCyclePresets = true
    @AppStorage("resonance.projectmd.fullscreenDefaultsConfigured") var projectMDefaultsConfigured = false
    @AppStorage("lyrics.provider.enabled") var lyricsProviderEnabled = false
    @AppStorage("lyrics.provider.name") var lyricsProviderName = ""
    @AppStorage("lyrics.provider.endpoint") var lyricsProviderEndpoint = ""
    @AppStorage("lyrics.provider.requestMethod") private var lyricsProviderRequestMethodRaw = LyricsRequestMethod.get.rawValue
    @AppStorage("lyrics.provider.responseFormat") private var lyricsProviderResponseFormatRaw = LyricsResponseFormat.json.rawValue
    @AppStorage("lyrics.provider.titleParameter") var lyricsProviderTitleParameter = "title"
    @AppStorage("lyrics.provider.artistParameter") var lyricsProviderArtistParameter = "artist"
    @AppStorage("lyrics.provider.albumParameter") var lyricsProviderAlbumParameter = "album"
    @AppStorage("lyrics.provider.durationParameter") var lyricsProviderDurationParameter = "duration"
    @AppStorage("lyrics.provider.plainResponsePath") var lyricsProviderPlainResponsePath = "plainLyrics"
    @AppStorage("lyrics.provider.syncedResponsePath") var lyricsProviderSyncedResponsePath = "syncedLyrics"
    @AppStorage("lyrics.provider.authorizationMode") private var lyricsProviderAuthorizationModeRaw = LyricsAuthorizationMode.none.rawValue
    @AppStorage("lyrics.provider.authorizationName") var lyricsProviderAuthorizationName = ""
    @AppStorage("lyrics.provider.userAgent") var lyricsProviderUserAgent = "MeiKyo/1.0 (https://github.com/thnikkaman/Resonance)"
    ...
    @Published var lyricsProviderToken: String {
        didSet { KeychainCredentialStore.save(lyricsProviderToken, account: "lyrics-provider-token") }
    }

    var visualizerTelemetryAvailable: Bool {
        VisualizerTelemetryConfiguration.isCompiledIn
    }

    init() {
        streamPassword = KeychainCredentialStore.load(account: "remote-library-password") ?? ""
        lyricsProviderToken = KeychainCredentialStore.load(account: "lyrics-provider-token") ?? ""
        customThemeImageData = try? Data(contentsOf: Self.customThemeImageURL)

        // Remote server connections are HTTPS-only. Keep the persisted key for
        // upgrade compatibility, but normalize older HTTP-enabled settings.
        streamUseHTTPS = true
    ...
    var lyricsProviderRequestMethod: LyricsRequestMethod {
        get { LyricsRequestMethod(rawValue: lyricsProviderRequestMethodRaw) ?? .get }
        set { lyricsProviderRequestMethodRaw = newValue.rawValue; objectWillChange.send() }
    }

    var lyricsProviderResponseFormat: LyricsResponseFormat {
        get { LyricsResponseFormat(rawValue: lyricsProviderResponseFormatRaw) ?? .json }
        set { lyricsProviderResponseFormatRaw = newValue.rawValue; objectWillChange.send() }
    }

    var lyricsProviderAuthorizationMode: LyricsAuthorizationMode {
        get { LyricsAuthorizationMode(rawValue: lyricsProviderAuthorizationModeRaw) ?? .none }
        set { lyricsProviderAuthorizationModeRaw = newValue.rawValue; objectWillChange.send() }
    }

    var lyricsProviderConfiguration: LyricsProviderConfiguration? {
        guard lyricsProviderEnabled else { return nil }
        return LyricsProviderConfiguration(
            name: lyricsProviderName.trimmingCharacters(in: .whitespacesAndNewlines),
            endpoint: lyricsProviderEndpoint.trimmingCharacters(in: .whitespacesAndNewlines),
            requestMethod: lyricsProviderRequestMethod,
            responseFormat: lyricsProviderResponseFormat,
            titleParameter: lyricsProviderTitleParameter.trimmingCharacters(in: .whitespacesAndNewlines),
            artistParameter: lyricsProviderArtistParameter.trimmingCharacters(in: .whitespacesAndNewlines),
            albumParameter: lyricsProviderAlbumParameter.trimmingCharacters(in: .whitespacesAndNewlines),
            durationParameter: lyricsProviderDurationParameter.trimmingCharacters(in: .whitespacesAndNewlines),
            plainResponsePath: lyricsProviderPlainResponsePath.trimmingCharacters(in: .whitespacesAndNewlines),
            syncedResponsePath: lyricsProviderSyncedResponsePath.trimmingCharacters(in: .whitespacesAndNewlines),
            authorizationMode: lyricsProviderAuthorizationMode,
            authorizationName: lyricsProviderAuthorizationName.trimmingCharacters(in: .whitespacesAndNewlines),
            authorizationToken: lyricsProviderToken,
            userAgent: lyricsProviderUserAgent.trimmingCharacters(in: .whitespacesAndNewlines)
        )
    }
