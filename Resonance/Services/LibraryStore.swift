...
final class LibraryStore: ObservableObject {
    ...
    func updateTrackMetadata(
        ...
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.trackSave.begin",
            details: [
                "track": String(trackID.uuidString.prefix(8)),
                "artworkBytes": String(artworkData?.count ?? 0),
                "replaceArtwork": String(replaceArtwork)
            ]
        )
        guard let index = tracks.firstIndex(where: { $0.id == trackID }) else {
            ResonanceDiagnostics.shared.recordDeferredAlways(
                "library.metadata.trackSave.rejected",
                details: ["reason": "track-not-in-library"]
            )
        ...
        guard let url = track.fileURL, !track.isRemote else {
            ResonanceDiagnostics.shared.recordDeferredAlways(
                "library.metadata.trackSave.rejected",
                details: ["reason": track.isRemote ? "remote-track" : "missing-file-url"]
            )
        ...
        if let failure = result.failures.first {
            ResonanceDiagnostics.shared.recordDeferredAlways(
                "library.metadata.trackSave.failed",
                details: [
                    "track": String(trackID.uuidString.prefix(8)),
                    "failureCount": String(result.failures.count),
                    "error": failure
                ]
            )
            return failure
        ...
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.trackSave.rereadRequested",
            details: ["track": String(trackID.uuidString.prefix(8))]
        )
        return nil
    }
    ...
    func updateArtistMetadata(
        ...
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.artistSave.begin",
            details: [
                "artistTrackCount": String(artist.albums.flatMap(\.tracks).count),
                "artworkBytes": String(artworkData?.count ?? 0),
                "replaceArtwork": String(replaceArtwork),
                "clearArtworkOverride": String(clearArtworkOverride)
            ]
        )
        ...
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.artistSave.end",
            details: [
                "requestCount": String(requests.count),
                "successCount": String(writableIDs.count),
    ...
    func updateAlbumMetadata(
        ...
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.albumSave.begin",
            details: [
                "trackCount": String(trackIDs.count),
                "artworkBytes": String(artworkData?.count ?? 0),
                "replaceArtwork": String(replaceArtwork)
            ]
        )
        ...
        await refreshMetadataFiles(from: writeResult, requests: requests)
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.albumSave.end",
            details: [
                "requestCount": String(requests.count),
                "successCount": String(writableIDs.count),
                "failureCount": String(failures.count)
    ...
    private func refreshMetadataFiles(
        ...
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.reread.begin",
            details: [
                "requestCount": String(requests.count),
                "successfulCount": String(result.successfulIDs.count)
            ]
        )
        for request in requests where result.successfulIDs.contains(request.id) {
            ResonanceDiagnostics.shared.recordDeferredAlways(
                "library.metadata.reread.request",
                details: ["track": String(request.id.uuidString.prefix(8))]
            )
            await refreshDownloadedTrack(at: request.url)
            ResonanceDiagnostics.shared.recordDeferredAlways(
                "library.metadata.reread.completed",
                details: ["track": String(request.id.uuidString.prefix(8))]
            )
        }
        ResonanceDiagnostics.shared.recordDeferredAlways(
            "library.metadata.reread.end",
