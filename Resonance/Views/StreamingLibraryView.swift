...
struct StreamingLibraryView: View {
    ...
    // Launch-time preparation owns catalog activation. Keep this view usable
    // while the detached browse projection finishes instead of making the
    // first tab switch wait for a second activation/prewarm cycle.
    @State private var browseReady = true
    ...
    private var selectedDownloadTracks: [RemoteTrackItem] {
        selectedArtistTracks + selectedAlbumTracks
    }
    ...
    private func handleDownloadTap() {
        if selectedDownloadTracks.isEmpty {
            showingDownloadPrompt = true
        } else {
            requestDownload(selectedDownloadTracks)
        }
    }
    ...
    var body: some View {
        ...
        .alert("Download Streaming Library", isPresented: $showingDownloadPrompt) {
            Button("Download Entire Library") {
                requestDownload(remote.tracks)
            }
            Button("Select Individual Artists") {
                remote.grouping = .artists
                downloadSelectionMode = true
            }
            Button("Cancel", role: .cancel) {}
        } message: {
            Text("Download the entire remote library, or select individual artists first.")
        }
        ...
        .task(id: serverConfigurationKey) {
            // Server activation is a launch-time background operation. This
            // task handles configuration changes without blocking tab entry
            // or starting another browse projection here.
        ...
        ZStack(alignment: .top) {
            if downloads.pendingReplacementCount > 0 {
                ...
                RemoteDownloadBanner(
                    ...
                    liveProgress: downloads.liveProgress,
...
private struct RemoteDownloadBanner: View {
    ...
    @ObservedObject var liveProgress: RemoteDownloadLiveProgress
    let cancel: () -> Void
    let cancelTrack: (UUID) -> Void
    let requeueTrack: (UUID) -> Void
    @State private var isExpanded = false

    private var activeProgress: RemoteDownloadProgress? {
        liveProgress.snapshot
    }

    private func displayedQueueItem(_ item: RemoteDownloadProgress) -> RemoteDownloadProgress {
        guard activeProgress?.id == item.id, let activeProgress else { return item }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                Group {
                    if let activeProgress, activeProgress.total > 0 {
                        ProgressView(
                            value: Double(activeProgress.completed),
                            total: Double(activeProgress.total)
                        )
                    } else {
                        ProgressView()
                    }
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Downloading \(min(completed + 1, total)) of \(total)")
                        .font(.caption.weight(.semibold))
                    Text(activeProgress?.title ?? "Preparing next download…")
                        .font(.caption2)
                        .lineLimit(1)
                        .foregroundStyle(.secondary)
                    if let activeProgress, activeProgress.total > 0 {
                        Text("\(ByteCountFormatter.string(fromByteCount: Int64(activeProgress.completed), countStyle: .file)) of \(ByteCountFormatter.string(fromByteCount: Int64(activeProgress.total), countStyle: .file))")
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 2) {
                        ForEach(queue) { item in
                            RemoteDownloadQueueRow(
                                item: displayedQueueItem(item),
                                cancel: cancelTrack,
                                requeue: requeueTrack
                            )
                        }
                    }
                }
