...
struct RootView: View {
  ...
  private var selectedTab: AppTab {
    ...
    var body: some View {
        ...
        .sheet(
            isPresented: Binding(
                get: { !settings.privacyPolicyAcknowledged },
                set: { _ in }
            )
        ) {
            ResonancePrivacyPolicyConsentView {
                settings.privacyPolicyAcknowledged = true
            }
...
private struct ResonancePrivacyPolicyConsentView: View {
    @EnvironmentObject private var settings: AppSettings
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Text("Welcome to MeiKyo 鳴響")
                        .font(.title.bold())
                        .frame(maxWidth: .infinity, alignment: .center)

                    Text("Please review the MeiKyo Privacy Policy before using the app. MeiKyo keeps local music and library data on this iPhone unless you choose a remote server or provider feature.")
                        .fixedSize(horizontal: false, vertical: true)

                    Text("Network features are user-controlled. A configured music server receives the requests needed for browsing, playback, downloads, and other enabled actions. Optional lyrics, artwork, and diagnostic services have their own disclosures and controls.")
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    Toggle(isOn: $hasAgreed) {
                        Text("I have read and agree to the MeiKyo Privacy Policy.")
                            .fixedSize(horizontal: false, vertical: true)
                    }
                    .tint(settings.accentColor)
