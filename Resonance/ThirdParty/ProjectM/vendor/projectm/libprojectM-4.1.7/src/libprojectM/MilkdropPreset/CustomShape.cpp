...
namespace {
// Keep a single pathological preset from monopolizing the iOS render loop.
// This is an engine-wide safety budget; individual MilkDrop files remain
// untouched, and the capped value is also exposed through num_inst so their
// per-frame expressions see the actual rendered count.
constexpr int kMaximumCustomShapeInstances = 128;
}
...
void CustomShape::Initialize(PresetFileParser& parsedFile, int index)
{
    ...
    m_instances = std::clamp(parsedFile.GetInt(shapecodePrefix + "num_inst", m_instances),
                             0,
                             kMaximumCustomShapeInstances);
...
void CustomShape::Draw()
{
    ...
    glEnable(GL_BLEND);
    m_untexturedFillBatch.clear();

    bool hasUntexturedBatch = false;
    bool untexturedBatchAdditive = false;
    auto flushUntexturedBatch = [&]() {
        if (m_untexturedFillBatch.empty()) {
            hasUntexturedBatch = false;
            return;
        }

        glBlendFunc(GL_SRC_ALPHA, untexturedBatchAdditive ? GL_ONE : GL_ONE_MINUS_SRC_ALPHA);
        ...
        glBufferData(
            GL_ARRAY_BUFFER,
            static_cast<GLsizeiptr>(sizeof(TexturedPoint) * m_untexturedFillBatch.size()),
            nullptr,
            GL_STREAM_DRAW
        );
        glBufferSubData(
            ...
            0,
            static_cast<GLsizeiptr>(sizeof(TexturedPoint) * m_untexturedFillBatch.size()),
            m_untexturedFillBatch.data()
        );
        glBindVertexArray(m_vaoIdUntextured);
        glDrawArrays(GL_TRIANGLES, 0, static_cast<GLsizei>(m_untexturedFillBatch.size()));
        glBindVertexArray(0);
        m_untexturedFillBatch.clear();
        hasUntexturedBatch = false;
    };
    ...
    for (int instance = 0; instance < m_instances; instance++)
    {
        ...
        m_vertexData.resize(static_cast<size_t>(sides + 2));
        ...
        {
            const bool hasBorder = *m_perFrameContext.border_a > 0.0001f;
            if (!hasBorder)
            {
                const bool additive = static_cast<int>(*m_perFrameContext.additive) != 0;
                if (hasUntexturedBatch && additive != untexturedBatchAdditive) {
                    flushUntexturedBatch();
                }
                if (!hasUntexturedBatch) {
                    untexturedBatchAdditive = additive;
                    hasUntexturedBatch = true;
                }

                for (int i = 1; i < sides + 1; i++)
                {
                    m_untexturedFillBatch.push_back(vertexData[0]);
                    m_untexturedFillBatch.push_back(vertexData[i]);
                    m_untexturedFillBatch.push_back(vertexData[i + 1]);
                }
                continue;
            }
