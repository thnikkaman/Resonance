...
void ProjectM::RenderFrame()
{
    ...
    if (m_transition != nullptr && m_transitioningPreset != nullptr)
    {
        if (m_transition->IsDone())
        {
            ...
            m_transition.reset();
            m_transitionFirstCompositePending = false;
        }
        else if (!m_holdOutgoingFrame)
        {
            // The active preset is the outgoing side of a soft transition.
            // Its output texture already contains a complete frame, so keep
            // that frame stable while only the incoming preset evolves. This
            // preserves the transition shader and avoids rendering two full
            // MilkDrop presets for every displayed transition frame.
            m_transitioningPreset->RenderFrame(audioData, renderContext);
            renderActivePreset = false;
        }
    }


    // ToDo: Call the to-be-implemented render method in Renderer
    if (renderActivePreset)
    {
    ...
    if (m_transition != nullptr && m_transitioningPreset != nullptr)
    {
        ...
        {
        // The output framebuffer is explicitly bound above. The prior check
        // read stale errors left by the incoming preset before the compositor
        // ran, then forced synchronous GPU/CPU round trips. Draw directly and
        // retain only an asynchronous first-composite boundary.
        m_transition->Draw(*m_activePreset, *m_transitioningPreset, renderContext, audioData);
        if (m_transitionFirstCompositePending)
        {
            resonance_diagnostics_record("projectm.transition.first_composite", "result", "drawn_without_driver_poll");
            m_transitionFirstCompositePending = false;
        }
        }
#else
...
void ProjectM::StartPresetTransition(std::unique_ptr<Preset>&& preset, bool hardCut)
{
    ...
    else
    {
        ...
        m_transitionFirstCompositePending = true;
    }
}
