...
private:
    ...
    // Heavy presets can draw hundreds or thousands of instances. Keep this
    // storage alive across instances and frames instead of allocating vertex
    // arrays in the per-instance draw loop.
    std::vector<TexturedPoint> m_vertexData;
    std::vector<ShapeVertex> m_borderPoints;
    std::vector<TexturedPoint> m_untexturedFillBatch;
