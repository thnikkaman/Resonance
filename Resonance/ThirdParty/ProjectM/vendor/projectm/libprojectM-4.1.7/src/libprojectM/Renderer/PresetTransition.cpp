...
void PresetTransition::Restart()
{
    m_transitionStartTime = std::chrono::system_clock::now();
    m_lastFrameTime = m_transitionStartTime;
    m_hasRenderedFirstFrame = false;
}
                            ...
                            const libprojectM::Audio::FrameAudioData& audioData)
{
    ...
    // Loading a preset and drawing its first frame are separate operations.
    // Keep the first compositor sample explicitly at the outgoing image so
    // the new preset cannot flash for one frame before the blend begins.
    if (!m_hasRenderedFirstFrame)
    {
        linearProgress = 0.0;
        cosineProgress = 0.0;
        bicubicProgress = 0.0;
        m_hasRenderedFirstFrame = true;
    }
