...
public:
    ...
    void SetTargetFramebuffer(unsigned int framebuffer);

    void SetLyricTexture(const uint8_t* rgbaPixels, int width, int height);

    void SetLyricProgress(float progress);

    void ClearLyricTexture();
...
private:
    ...
    unsigned int m_targetFramebuffer{0};  //!< Drawable framebuffer supplied by the embedding application.
    ...
    std::unique_ptr<Renderer::Framebuffer> m_outputFramebuffer;                   //!< Complete-frame offscreen composite before presentation.
    std::unique_ptr<Renderer::MilkdropText> m_milkdropText;                       //!< Original MilkDrop title mesh and feedback injector.
    ...
    bool m_transitionFirstCompositePending{false};                                 //!< Records the first composite without synchronously polling GLES errors.
