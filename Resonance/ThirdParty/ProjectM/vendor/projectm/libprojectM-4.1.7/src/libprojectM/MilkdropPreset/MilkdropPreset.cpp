...
void MilkdropPreset::RenderFrame(const libprojectM::Audio::FrameAudioData& audioData, const Renderer::RenderContext& renderContext)
{
    ...
    // MilkDrop 2 burns a completed title into the feedback surface at this
    // exact stage. The next frame's warp and shaders then manipulate the text
    // as visualization content instead of treating it as a UI overlay.
    if (renderContext.milkdropText != nullptr &&
        renderContext.milkdropText->ShouldBurnIntoFeedback())
    {
        renderContext.milkdropText->Draw(renderContext.viewportSizeX,
                                         renderContext.viewportSizeY,
                                         renderContext.time, 1.0f);
    }
